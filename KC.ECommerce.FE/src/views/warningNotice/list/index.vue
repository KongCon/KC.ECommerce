<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      />
      <el-input
        v-model="listQuery.emailContent"
        placeholder="请输入邮件内容"
        style="width: 200px;"
        class="filter-item"
        @keyup.enter.native="handleFilter"
    ...
    >
        ...
        <template slot-scope="scope">
          <el-form label-position="left" inline class="table-expand">
            ...
            <el-form-item label="邮件内容">
              <el-input type="textarea" :autosize="{ minRows: 2, maxRows: 5}"  v-model="scope.row.emailContent"></el-input>
            </el-form-item>
          </el-form>
        </template>
      </el-table-column>
      ...
      <el-table-column label="发送数量" align="center" width="80">
        <template slot-scope="scope">
          <span>{{ scope.row.sendCount }}</span>
        </template>
      </el-table-column>
      <el-table-column label="发送时间" align="center" width="135">
        <template slot-scope="scope">
      ...
      <el-table-column label="发送邮件" align="center" width="80">
        <template slot-scope="scope">
          <span>{{ scope.row.isSendEmail }}</span>
        </template>
      </el-table-column>
      <el-table-column label="发送短信" align="center" width="80">
        <template slot-scope="scope">
          <span>{{ scope.row.isSendSms }}</span>
        </template>
      </el-table-column>
      <el-table-column label="配置类型" align="center" show-overflow-tooltip>
        <template slot-scope="scope">
    ...
    <el-dialog title="处理状态更改" :visible.sync="dialogUpdateVisible">
      <el-form :model="updateParas" label-width="70px" style="width: 500px; margin-left:50px;">
        <el-form-item label="短信内容" v-if="show.sms">
          <!-- <el-input v-model="updateParas.smsContent" autocomplete="off" maxlength="70"></el-input> -->
          <el-input type="text" placeholder="请输入内容" v-model="updateParas.smsContent" maxlength="70" show-word-limit></el-input>
        </el-form-item>
        <el-form-item label="邮件内容" v-if="show.email">
          ...
          <el-input type="textarea" :autosize="{ minRows: 2, maxRows: 5}" placeholder="请输入内容" v-model="updateParas.emailContent" maxlength="500" show-word-limit></el-input>
        </el-form-item>
      </el-form>
